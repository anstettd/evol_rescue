...
slope.summary <- read_csv("data/snp_change_2/mean_median_S_env578.csv") %>% dplyr::select(Site, Median, Mean)
...
library(sfsmisc)
f.robftest(rob.mod_S, var="Median") #Robust Reg P-value 0.001231
...
ggplot(slope_pop, aes(x=Median, y=mean.r.recovery)) + 
  ...
  scale_y_continuous(name="Mean Pop. Growth after Drought")+#,limits = c(-0.5, 2))+#,breaks=c(0.5,1,1.5,2,2.5))+
