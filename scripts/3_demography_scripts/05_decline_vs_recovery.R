...
hist(demog_recovery$mean.lambda.drought)
hist(log(demog_recovery$mean.lambda.drought+0.5))
hist(demog_recovery$mean.lambda.recovery)
hist(log(demog_recovery$mean.lambda.recovery+0.5))

cor.test(demog_recovery$mean.lambda.drought, demog_recovery$mean.lambda.recovery, method = 'spearman')

cor.test(log(demog_recovery$mean.lambda.drought+0.5), log(demog_recovery$mean.lambda.recovery+0.5), method = 'spearman')
...
ggplot(demog_recovery, aes(x=log(mean.lambda.drought+0.5), y=log(mean.lambda.recovery+0.5))) + 
  geom_point(aes(fill=demog_recovery$Lat.Color), shape=21, size =6)+
  geom_hline(yintercept = 0, linetype = "dotted", color = "black", size = 0.7) +
  geom_vline(xintercept = 0, linetype = "dotted", color = "black", size = 0.7) +
  scale_y_continuous(name="Log Mean Lambda after Drought")+#, limits=c(-0.3,2.5),breaks=seq(0,2.5,0.5))+
  scale_x_continuous(name="Log Mean Lambda during Drought")+
  #, limits=c(0.2,0.35),breaks=seq(0.1,0.35,0.05))+
  scale_fill_manual(name = "Latitude (°N)",labels=round(demog_recovery$Latitude,1), values=as.character(demog_recovery$Lat.Color)) +
  theme_classic() + theme(
    axis.text.x = element_text(size=20, face="bold"),
