...
#Decline only
ggplot(dat_decline, aes(x=Year, y=r)) +
  ...
  scale_y_continuous(name="r")+ scale_x_continuous(name="Year")+
...
#*******************************************************************************
### 4. Visualize decline over time for exemplar site (Fig 1D)
#*******************************************************************************

#Little Jamison
dat_little <- dat %>% filter(Site=="Little Jameson Creek") %>% filter(Year<2015)
plot_3 <- ggplot(dat_little, aes(x=Year, y=r)) + geom_point(size=4,shape=21,fill="#9BD7A4") +
  ...
  scale_y_continuous(name="r")+ scale_x_continuous(name="")+
  geom_hline(yintercept=0, linetype="dotted",size=0.8) + theme_classic() + theme(
    #facet_wrap(~Site.Lat, scale="free") + theme_classic() + theme(
